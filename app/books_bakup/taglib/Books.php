...
// +----------------------------------------------------------------------
namespace app\books\taglib;

use think\template\TagLib;

class Books extends TagLib
{
    /**
     * 定义标签列表
     */
    ...
    public function tagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\books\service\ApiService::articles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
parse;
    ...
    public function tagTagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\books\service\ApiService::tagArticles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
<?php
if(!empty({$cid})){
    \$__BREADCRUMB_ITEMS__ = \app\books\service\ApiService::breadcrumb({$cid},{$self});
?>
...
parse;
    ...
    public function tagCategories($tag, $content)
    {
        ...
        $returnVarName = 'books_categories_data';
        $where         = '""';
        if (!empty($tag['where']) && strpos($tag['where'], '$') === 0) {
            $where = $tag['where'];
        }
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\books\service\ApiService::categories([
    'where'   => {$where},
    'order'   => '{$order}',
]);
...
parse;
    ...
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $returnVarName = 'books_sub_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\books\service\ApiService::subCategories({$categoryId});
...
parse;
    ...
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $returnVarName = 'books_all_sub_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\books\service\ApiService::allSubCategories({$categoryId});
 ?>
<volist name="{$returnVarName}" id="{$item}">
{$content}
</volist>
