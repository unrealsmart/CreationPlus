...
class BooksPostModel extends Model
{
    ...
    {
        return $this->belongsToMany('BooksCategoryModel', 'books_category_post', 'category_id', 'post_id');
    }
    ...
    {
        return $this->belongsToMany('BooksTagModel', 'books_tag_post', 'tag_id', 'post_id');
    }
     ...
     * @return $this
     */
    public function adminAddBooks($data, $categories)
    {
     ...
     * @return $this
     */
    public function adminEditBooks($data, $categories)
    {
    ...
    public function addTags($keywords, $articleId)
    {
        ...
        if (!empty($keywords)) {
            ...
            $oldTagIds = Db::name('books_tag_post')->where('post_id', $articleId)->column('tag_id');

            foreach ($keywords as $keyword) {
                $keyword = trim($keyword);
                if (!empty($keyword)) {
                    $findTag = $booksTagModel->where('name', $keyword)->find();
                    if (empty($findTag)) {
                        $tagId = $booksTagModel->insertGetId([
                            'name' => $keyword
                        ]);
                    } else {
                        $tagId = $findTag['id'];
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('books_tag_post')->where('post_id', $articleId)->delete();
            }
            ...
            if (!empty($shouldDeleteTagIds)) {
                Db::name('books_tag_post')->where(['post_id' => $articleId, 'tag_id' => ['in', $shouldDeleteTagIds]])->delete();
            }

            if (!empty($data)) {
                Db::name('books_tag_post')->insertAll($data);
            }
    ...
    public function adminDeletePage($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            if ($res) {
                ...
                $transStatus = false;
                try {
                    Db::name('books_post')->where(['id' => $id])->update([
                        'delete_time' => time()
                    ]);
            ...
            if ($res) {
                ...
                foreach ($res as $key => $value) {
                    ...
                    $recycleData[$key]['table_name']  = 'books_post';
                    $recycleData[$key]['name']        = $value['post_name'];
                ...
                $transStatus = false;
                try {
                    Db::name('books_post')->where(['id' => ['in', $ids]])
                        ->update([
                            'delete_time' => time()
                        ]);
    ...
    public function adminEditPage($data)
    {
        ...
        $routeModel->setRoute($data['post_alias'], 'books/Page/index', ['id' => $data['id']], 2, 5000);
