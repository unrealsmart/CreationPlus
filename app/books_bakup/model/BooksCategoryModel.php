...
//        }
        ...
        foreach ($categories as $item) {
            ...
            $item['url']     = cmf_url('books/List/index', ['id' => $item['id']]);;
    ...
    public function addCategory($data)
    {
        ...
        if ($result != false) {
            ...
            if (!empty($data['alias']) && !empty($id)) {
                $routeModel->setRoute($data['alias'], 'books/List/index', ['id' => $id], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'books/Article/index', ['cid' => $id], 2, 4999);
            }
    ...
    public function editCategory($data)
    {
        ...
        if (empty($oldCategory) || empty($newPath)) {
            ...
            $routeModel = new RouteModel();
            if (!empty($data['alias'])) {
                $routeModel->setRoute($data['alias'], 'books/List/index', ['id' => $data['id']], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'books/Article/index', ['cid' => $data['id']], 2, 4999);
            } else {
                $routeModel->deleteRoute('books/List/index', ['id' => $data['id']]);
                $routeModel->deleteRoute('books/Article/index', ['cid' => $data['id']]);
            }
