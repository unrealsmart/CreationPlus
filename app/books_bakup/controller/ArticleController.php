...
class ArticleController extends HomeBaseController
{
    public function index()
    {
        ...
        if (empty($categoryId)) {
            ...
            $category = $booksCategoryModel->where('id', $categoryId)->where('status', 1)->find();
        ...
        Db::name('books_post')->where(['id' => $articleId])->setInc('post_hits');


        hook('books_before_assign_article', $article);
    ...
    public function doLike()
    {
        ...
        if ($canLike) {
            Db::name('books_post')->where(['id' => $articleId])->setInc('post_like');
