...
class ApiService
{
    /**
     * 功能:查询文章列表,支持分页;<br>
     * 注:此方法查询时关联两个表books_category_post(category_post),books_post(post);在指定排序(order),指定查询条件(where)最好指定一下表别名
     ...
     * field:调用指定的字段@todo
     *   如只调用posts表里的id和post_name字段可以是post.id,post.post_name; 默认全部,
     *   此方法查询时关联两个表books_category_post(category_post),books_post(post);
     *   所以最好指定一下表名,以防字段冲突
    ...
    public static function articles($param)
    {
        ...
        $articles = $booksPostModel->alias('post')->field($field)
            ->join($join)
            ->where($where)
            ->where($paramWhere)
            ->order($order)
    ...
    /**
     * 功能:查询标签文章列表,支持分页;<br>
     * 注:此方法查询时关联两个表books_tag_post(tag_post),books_post(post);在指定排序(order),指定查询条件(where)最好指定一下表别名
     * @param array $param 查询参数<pre>
     * array(
     *  'tag_id'=>'',
     *  'where'=>'',
     ...
     * field:调用指定的字段@todo
     *   如只调用posts表里的id和post_name字段可以是post.id,post.post_name; 默认全部,
     *   此方法查询时关联两个表books_tag_post(category_post),books_post(post);
     *   所以最好指定一下表名,以防字段冲突
     * limit:数据条数,默认值为10,可以指定从第几条开始,如3,8(表示共调用8条,从第3条开始)
     * order:排序方式,如按posts表里的published_time字段倒序排列：post.published_time desc
     * where:查询条件,字符串形式,和sql语句一样,请在事先做好安全过滤,最好使用第二个参数$where的数组形式进行过滤,此方法查询时关联多个表,所以最好指定一下表名,以防字段冲突,查询条件(只支持数组),格式和thinkPHP的where方法一样,此方法查询时关联多个表,所以最好指定一下表名,以防字段冲突;
    ...
    public static function article($id)
    {
        ...
        return $booksPostModel->where($where)->find();
    }
    ...
    public static function category($id)
    {
        ...
        return $booksCategoryModel->where($where)->find();
    }
    ...
    public static function subCategories($categoryId)
    {
        ...
        return $booksCategoryModel->where($where)->select();
    }
    ...
    public static function allSubCategories($categoryId)
    {
        ...
        if ($categoryId !== 0) {
            $category = $booksCategoryModel->field('path')->where('id', $categoryId)->find();
    ...
    public static function breadcrumb($categoryId, $withCurrent = false)
    {
        ...
        $path = $booksCategoryModel->where(['id' => $categoryId])->value('path');
        ...
        if (!empty($path)) {
            ...
            if (!empty($parents)) {
                $data = $booksCategoryModel->where(['id' => ['in', $parents]])->order('path ASC')->select();
            }
        }
